	...
	<view>
		...
		<view class="category-list">
			...
			<scroll-view  scroll-y="true" class="left" >
                <view class="row" v-for="(category,index) in categoryParent" :key="category.id" :class="[category.id==showCategoryIndex?'on':'']" @tap="showCategory(category.id)">
					<view class="text">
			...
			<scroll-view  scroll-y="true" class="right" >
				<view class="category">
					<view class="banner">
						<image :src="currentCategory.picUrl"></image>
					</view>
					<view class="list">
						<view class="box" v-for="(box,i) in categoryList.currentSubCategory" :key="i" @tap="toCategory(box)">
...
<script>
	...
	export default {
		...
		onLoad() {
			...
			uni.request({
				url: 'https://www.xinghuohan.cn/xhh-wx-api/wx/catalog/index',
				method: 'GET',
				data: {},
				success: res => {
					this.categoryParent = res.data.data.categoryList;
				},
				fail: () => {},
				complete: () => {
					this.showCategory(this.categoryParent[0].id);
				}
			});
		},
		methods: {
			...
			showCategory(index){
				...
				uni.request({
					url: 'https://www.xinghuohan.cn/xhh-wx-api/wx/catalog/current?id='+index,
					method: 'GET',
					data: {},
					success: res => {
			...
			toCategory(e){
				...
				uni.navigateTo({
					...
					url: '/pages/product/list?cid='+e.id
				});
			},
			//搜索跳转
			toSearch(){
